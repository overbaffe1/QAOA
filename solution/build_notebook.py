...
# chunk=64 без GPU: те же 500 инстансов кусками по 64, иначе батч 500 на
# backward требует больше 3 ГБ оперативки и процесс убивает OOM-киллер.
CHUNK_LABELS = 0 if torch.cuda.is_available() else 64
...
На инференсе к выходу сети добавляется «полировка» — 5 рестартов
(сеть, сеть+шум, три случайных) по 250 шагов Adam + 100 шагов доводки мелким
`lr`, для каждого `h` остаётся лучший набор углов по P(ground)
(тысячи запусков схемы заменяются сотнями шагов).
"""))
...
**4. Инференс.** `f(h)` -> (gamma, beta) + **поинстансная полировка**:
5 рестартов (рестарт 0 — предсказание сети, 1 — сеть + шум N(0, 0.3),
2..4 — случайные углы в полном диапазоне gamma ~ U[0, 2pi], beta ~ U[0, pi]),
каждый — 250 шагов Adam (lr 0.05) + 100 шагов доводки мелким lr (0.01);
для каждого инстанса остаётся лучший набор углов по P(ground) (best-of).
На T4 это ~5-8 мин при лимите 10 мин; на CPU — часы, поэтому для локальной
проверки без GPU удобнее `solution/infer.py --profile fast`.

**4b. Память.** Метки и полировка считаются батчем из 500 инстансов
(на GPU это эффективно). Если RAM/VRAM мало, `generate_labels.py`
поддерживает `run(..., chunk=64)` — те же инстансы кусками по 64
(результат эквивалентен, память пропорциональна размеру куска).
