...
def get_results():
    path = os.path.join(ROOT, "data", "results.json")
...
pdf.bullets([
    ...
    ("Идея решения: ", "гибрид «ML-тёплый старт + короткая классическая доводка»: "
     "сеть задаёт стартовые углы, 40 шагов Adam по конкретному h добирают "
     "остальное."),
...
pdf.bullets([
    ...
    ("Шаг 2 — полировка: ", "от предсказания сети 40 шагов Adam поинстансно "
     "(каждый h дорабатывает свои углы, loss = -P(ground))."),
    "Тысячи запусков схемы -> десятки; качество почти полное: сеть + 40 шагов "
     "теряют доли процента P(ground) против полной оптимизации.",
    (f"Время: ~2-3 мин на CPU / секунды на GPU ", f"для всех 500 h_test "
     f"(лимит 10 мин). Детерминировано -> воспроизводимо."),
...
pdf.bullets([
    ...
    ("Локально: ", "pip install -r requirements.txt; "
     "generate_labels.py -> train.py -> infer.py."),
